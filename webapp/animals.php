...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name) || empty($species) || empty($category) || empty($sex)) {
        ...
        /* Save slug to DB so animals.php can link to _dynamic_animal.php?slug=...
           The ALTER TABLE above guarantees the column exists. */
        try {
        ...
        if (empty($error)) {
            $success = 'Animal added successfully! Their page is now live on the Animals listing.';
            if (!empty($photoError)) {
                $error = $photoError;
            }
        }
